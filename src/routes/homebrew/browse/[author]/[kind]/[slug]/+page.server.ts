// /homebrew/browse/[author]/[kind]/[slug] — detail page. URL slot 1 is the
// author username (resolved to user_id) so slug collisions across authors
// disambiguate naturally. Visibility:
...
export const load: PageServerLoad = async ({ params, locals, url }) => {
  ...
  // Visibility gate. Owner always sees their own; others need !private.
  const isOwner = row.ownerUserId === locals.user.id;
  if (!isOwner && row.visibility === 'private') throw error(404, 'not found');

  // Subscriber count + whether the viewer already subscribes.

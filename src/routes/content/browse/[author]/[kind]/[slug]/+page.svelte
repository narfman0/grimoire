<script lang="ts">
  ...
  /** Monster rows get a full statblock render via monsterDerive(). The cast
   *  is safe because the row's `data` JSON came through MonsterDataSchema on
   *  write, and monsterDerive() is permissive about unknown fields. */
  $: monster = data.item.kind === 'monster' ? monsterDerive(data.item.data as Record<string, unknown>) : null;
</script>

<svelte:head><title>{data.item.name} · Content · Grimoire</title></svelte:head>

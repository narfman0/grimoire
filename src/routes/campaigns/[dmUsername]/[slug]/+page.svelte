    ...
    <ul class="mb-4 divide-y divide-slate-800">
      {#each data.characters as character (character.id)}
        <li class="flex items-center justify-between gap-3 py-2">
          <CharacterCard
            name={character.name}
            href={character.ownerUsername && character.slug
              ? characterUrl(character.ownerUsername, character.slug)
              : undefined}
...
<section class="mb-6 rounded-lg border border-slate-800 bg-slate-900/40 p-4 text-sm">
  <div class="flex items-center justify-between">
    ...
    <a class="rounded border border-slate-700 px-3 py-1 text-sm hover:border-emerald-600 hover:text-emerald-200" href={encountersUrl(data.campaign.dmUsername, data.campaign.slug)}>
      Open encounters →
    </a>
  </div>
</section>

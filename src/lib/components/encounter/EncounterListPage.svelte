<!--
  Encounter list for a campaign. Shared by both campaign URL schemes
  (/c/[code]/encounters and /campaigns/[dmUsername]/[slug]/encounters) —
  they differ only in link targets, which come in as props.
-->
...
<script lang="ts">
  ...
  export let data: EncounterListData;
  ...
  async function deleteEncounter(id: string) {
    if (!(await confirmDialog({ title: 'Delete this encounter?', danger: true }))) return;
    busy = true;
    try {
...
<header class="mb-6 flex items-baseline justify-between">
  ...
  <a class="text-xs text-slate-400 hover:text-slate-200" href={campaignHref}>
    ← back to campaign
  </a>
</header>
      ...
      <ul class="divide-y divide-slate-800 rounded-lg border border-slate-800 bg-slate-900/30">
        {#each items as enc (enc.id)}
          <li class="flex items-center justify-between gap-3 px-4 py-3 text-sm">
            <div class="flex-1">
              <a class="font-medium hover:text-emerald-300" href={encounterHref(enc.id)}>
                {enc.name}
              </a>

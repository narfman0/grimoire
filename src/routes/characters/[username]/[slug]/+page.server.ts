...
export const load: PageServerLoad = async ({ params, locals }) => {
  ...
  // agnostic now; combat context (live encounter) is auto-detected below.
  ...
  // across ALL its campaigns — the sheet is no longer pinned to one campaign.
  // Restrict to encounters whose campaign the viewer can see (owner/admin sees
  // all; others only campaigns they share) so we don't leak that a character
  // is in a live fight elsewhere. Most-recent wins if several are live.
  ...
  const visibleLiveEnc = access.canEdit
    ? liveEncRows
    : liveEncRows.filter((r) => access.sharedCampaignIds.includes(r.campaignId));
  const liveEncMatch =
    [...visibleLiveEnc].sort((a, b) => b.encCreatedAt.getTime() - a.encCreatedAt.getTime())[0] ??
    null;

  // The resolved live encounter's campaign supplies (a) the content-grant
  // scope so combat-relevant author grants resolve, and (b) the viewer's
  // DM-vs-player role for reveal projection. With no live encounter the
  // content lookup is owner-scoped (homebrew + global + subscriptions).
  const contentCampaignId = liveEncMatch?.campaignId;
  const viewerRole = liveEncMatch
    ? await getMembershipByCampaignId(locals.user.id, liveEncMatch.campaignId)
    : null;

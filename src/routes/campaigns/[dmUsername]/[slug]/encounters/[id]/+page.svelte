...
<header class="mb-6 flex items-baseline justify-between">
  ...
  <div class="flex items-center gap-3">
    {#if data.role === 'dm'}
      ...
      {/if}
    {/if}
    <a class="text-xs text-slate-400 hover:text-slate-200" href={encountersUrl(data.campaign.dmUsername, data.campaign.slug)}>
      ← all encounters
    </a>
  </div>
</header>
  ...
  {#if p}
    ...
    <section class="mb-6 rounded-lg border border-slate-700 bg-slate-900/50 p-4 text-sm">
      ...
      <div class="mb-1 flex items-center gap-2">
        ...
        {#if p.characterId && isPc && data.characterLinks[p.characterId]}
          <a class="text-[10px] text-slate-400 hover:text-emerald-300" href={characterUrl(data.characterLinks[p.characterId].username, data.characterLinks[p.characterId].slug)}>↗ sheet</a>
        {/if}
        <button

...
- **Don't** add a route without also exporting an `openapi` const from its `+server.ts` file. The spec is auto-generated by `GET /api/openapi.json` via `import.meta.glob` — each route participates by exporting `export const openapi: RouteOpenApi = { METHOD: { summary, body?, response? } }` (import `RouteOpenApi` from `$lib/server/api/openapi`). The spec is the public contract.

<script lang="ts">
  ...
  import TogglesPanel, { type ToggleRow } from '$lib/components/TogglesPanel.svelte';
  import SpellSlotsPanel from '$lib/components/SpellSlotsPanel.svelte';
  ...
  import { serializeDerivedClient } from '$lib/rules/serialize-derived';
  import type { CharacterSheetPageData } from '$lib/server/character-page';

  export let data: CharacterSheetPageData;
  ...
  $: charDoc = (liveDoc ?? data.document) as CharacterDocument | null;

  // Re-derive when charDoc changes. Initial render uses the server's
  // derived output (data.derived) so first paint isn't blank.
  $: derived = charDoc
    ? serializeDerivedClient(derive(charDoc, contentLookup))
    : data.derived;
  ...
  $: impliedConditions = charDoc ? impliedBy(charDoc.conditions) : new Map<string, string>();

  // Resolve a content row's display name + description for hover tooltips.
  function contentMeta(kind: string, slug: string): { name?: string; description?: string } {
    const row = contentLookup({ kind, slug });
    const cdata = (row?.data ?? {}) as Record<string, unknown>;
    return {
      name: row?.name,
      description: typeof cdata.description === 'string' ? cdata.description : undefined
    };
  }

  // Toggles enriched with their source content's name + description so
  // TogglesPanel can render a feat-style hover for each switch.
  $: toggleRows = (derived?.toggles ?? []).map(
    (t): ToggleRow => {
      const meta = contentMeta(t.sourceContent.kind, t.sourceContent.slug);
      return { ...t, sourceName: meta.name, description: meta.description };
    }
  );

  // The header subtitle (class/species/subclass/background) as hoverable
  // tokens. Each resolves to the content row's name + description.
  $: identityEntries = charDoc ? buildIdentityEntries(charDoc) : [];
  function buildIdentityEntries(doc: CharacterDocument): IdentityEntry[] {
    const entries: IdentityEntry[] = [];
    doc.classes.forEach((c, i) => {
      const cls = contentMeta('class', c.slug);
      entries.push({
        text: `${c.slug}${c.subclass ? ` (${c.subclass})` : ''} ${c.level}`,
        sep: i === 0 ? '' : ', ',
        name: cls.name ?? c.slug,
        kindLabel: `Class · level ${c.level}${c.subclass ? ` · ${c.subclass}` : ''}`,
        description: cls.description
      });
    });
    const sp = contentMeta('species', doc.species.slug);
    entries.push({
      text: doc.subspecies ? `${doc.species.slug} (${doc.subspecies.slug})` : doc.species.slug,
      sep: entries.length ? ' · ' : '',
      name: sp.name ?? doc.species.slug,
      kindLabel: doc.subspecies ? `Species · ${doc.subspecies.slug}` : 'Species',
      description: sp.description
    });
    if (doc.background) {
      const bg = contentMeta('background', doc.background.slug);
      entries.push({
        text: doc.background.slug,
        sep: ' · ',
        name: bg.name ?? doc.background.slug,
  ...
  // Spell-slot levels resolved into rows for SpellSlotsPanel.
  $: spellSlotRows = ((slots) =>
    Object.keys(slots)
      .map(Number)
      .sort((a, b) => a - b)
      .map((level) => ({ level, ...slots[level] })))(derived?.stats.spellSlots ?? {});
  ...
  $: favorites = new Set(charDoc?.favoriteActionIds ?? []);
  ...
  $: movementUsed = charDoc?.movementUsedThisRound ?? 0;
  $: movementRemaining = Math.max(0, walkSpeed - movementUsed);
  $: actionUsed = charDoc?.actionUsedThisRound === true;
  $: bonusUsed = charDoc?.bonusActionUsedThisRound === true;
  ...
  $: {
    if (isMyTurn && !prevIsMyTurn) {
      ...
      const needsReset =
        charDoc?.actionUsedThisRound ||
        charDoc?.bonusActionUsedThisRound ||
        charDoc?.reactionUsedThisRound ||
        (charDoc?.movementUsedThisRound ?? 0) > 0;
      if (needsReset) {
        patchDocument((d) => {
  ...
  async function spendHitDie(classSlug: string) {
    if (!charDoc || !derived) return;
    const cls = charDoc.classes.find((c) => c.slug === classSlug);
    if (!cls) return;
    const spent = charDoc.hitDiceSpent[classSlug] ?? 0;
    ...
    const conMod = abilityMod(charDoc.abilityScores.con);
  ...
  $: maxFeats = (() => {
    ...
    for (const c of charDoc.classes) {
  ...
  function describeFeatChoices(slug: string): string {
    const f = charDoc?.feats.find((x) => x.slug === slug);
  ...
  // ResourcesPanel dispatches just the id; look the max up here so the panel
  // stays a dumb presentational component.
  function adjustResourceById(id: string, delta: number) {
    const r = derived?.resources.find((x) => x.id === id);
    if (!r) return;
  ...
  let concDraft = '';
  $: reactionUsed = charDoc?.reactionUsedThisRound === true;
  ...
  $: preparedCount = charDoc?.spells.prepared.length ?? 0;
  ...
  // dodged a reactivity bug). Surface it here when charDoc.background is
  ...
  function resetSpellSlots(d: NonNullable<typeof charDoc>) {
    d.resourcesSpent ??= {};
...
<svelte:head>
  <title>{data.campaign ? `${data.character.name} — ${data.campaign.name}` : data.character.name}</title>
</svelte:head>
...
<header class="mb-6 flex items-start justify-between gap-4">
  ...
  <div class="relative shrink-0">
    ...
    >
      {#if charDoc?.portrait}
        <img src={charDoc.portrait} alt="portrait" class="h-full w-full object-cover object-top" />
      {:else}
  ...
  <div class="min-w-0 flex-1">
    ...
    {#if charDoc}
      <IdentityLine entries={identityEntries} />
    {:else}
  ...
  {#if data.campaign}
    <a class="shrink-0 text-xs text-slate-400 hover:text-slate-200" href={`/c/${data.campaign.code}`}>
...
{#if showPortraitPicker && charDoc}
  <div class="mb-6 rounded-lg border border-slate-700 bg-slate-900/60 p-4">
    ...
    <PortraitPicker
      characterId={data.character.id}
      currentPortrait={charDoc.portrait}
      on:select={(e) => selectPortrait(e.detail)}
    />
  </div>
{/if}

{#if editingMeta && charDoc}
...
{#if charDoc && derived}
    ...
    <section class="mb-6 rounded-lg border border-slate-800 bg-slate-900/30 p-4">
      <div class="mb-2 flex items-center justify-between gap-2">
        ...
        <a
          ...
          href={data.campaign
            ? `/c/${data.campaign.code}/encounters/${data.liveEncounter.id}`
            : encounterUrl(
                data.liveEncounter.campaign.dmUsername,
                data.liveEncounter.campaign.slug,
                data.liveEncounter.id
              )}
        >
      ...
      <ActionEconomyPanel
        ...
        {walkSpeed}
        {movementUsed}
        concentrating={charDoc.concentrating ?? null}
  ...
  <section class="mb-6 grid gap-4 rounded-lg border border-slate-800 bg-slate-900/30 p-4 md:grid-cols-2">
    ...
    <div>
      ...
      <div class="text-3xl font-semibold">
        {charDoc.currentHp} / {derived.stats.hp.max}
        {#if charDoc.tempHp > 0}
          <span class="ml-2 text-base text-emerald-300">+{charDoc.tempHp} temp</span>
        {/if}
      </div>
      ...
      {#if charDoc.currentHp === 0}
        {@const ds = charDoc.deathSaves ?? { successes: 0, failures: 0 }}
      ...
      {#if restingShort}
        <div class="mt-3 rounded border border-slate-700 bg-slate-950/40 p-2">
          ...
          <ul class="space-y-1 text-sm">
            {#each charDoc.classes as cls}
              {@const spent = charDoc.hitDiceSpent[cls.slug] ?? 0}
  ...
  {#if derived.availableActivations.length > 0}
    <ActivationsPanel
      ...
      {busy}
      concentratingLabel={charDoc.concentrating?.label ?? null}
    ...
    <section class="mb-6 rounded-lg border border-slate-800 bg-slate-900/30 p-4">
      ...
      {#if derived.stats.spellcastingAbility}
        <SpellcastingSummary
          ...
          known={charDoc.spells.known.length}
  ...
  {#each charDoc.classes.filter((c) => c.level >= 3 && !c.subclass) as cls (cls.slug)}
    ...
    <section class="mb-6 rounded-lg border border-emerald-800 bg-emerald-950/20 p-4">
      ...
      <div class="space-y-3 text-sm">
        {#if draft.needsSubclass}
          <div class="block">
            <span id="levelup-subclass-label" class="mb-1 block text-xs uppercase tracking-wide text-slate-500">Subclass</span>
            ...
            {:else}
              <select
                ...
                aria-labelledby="levelup-subclass-label"
  ...
  <section class="mb-6 rounded-lg border border-slate-800 bg-slate-900/30 p-4">
    <div class="mb-3 flex items-baseline justify-between">
      <h2 class="text-sm font-semibold text-slate-200">
        Feats
        <span class="ml-1 text-xs text-slate-500">
          {charDoc.feats.length} / {maxFeats} expected
          {#if charDoc.feats.length > maxFeats}
            <span class="text-amber-300">· over budget — DM check</span>
          {:else if charDoc.feats.length < maxFeats}
            <span class="text-slate-600">· {maxFeats - charDoc.feats.length} slot(s) free</span>
          {/if}
        </span>
      </h2>
      <button
    ...
    {#if charDoc.feats.length === 0 && !showFeatPicker}
    ...
    {:else if charDoc.feats.length > 0}
      <ul class="mb-3 divide-y divide-slate-800 rounded border border-slate-800">
        {#each charDoc.feats as f (`${f.slug}|${f.authorUserId ?? ''}`)}
        ...
        <div class="flex gap-2">
          ...
          >
            ...
            {#each data.featOptions as opt}
              {@const taken = charDoc.feats.some(
